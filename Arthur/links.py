...
urlpatterns = patterns('Arthur.links',
    url(r'^game/$', 'game'),
    url(r'^pa_forums/$', 'pa_forums'),
    url(r'^dlr_forums/$', 'dlr_forums'),
...
@menu("Planetarion", "PA Forums")
class pa_forums(loadable):
    def execute(self, request, user):
        return HttpResponseRedirect("http://pirate.planetarion.com")

@menu("DLR", "DLR Forums")
class dlr_forums(loadable):
    def execute(self, request, user):
        return HttpResponseRedirect("http://progression-uk.com/DLR/forum/index.php")
